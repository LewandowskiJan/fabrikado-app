export const PLAYERS_NUMBER: number = 200;

// UNIVERSE CONFIG
export const ACCELERATION: number = 0.5;
export const UNIVERSE_NUMBER: number = 1;

// GALAXIES CONFIG
export const GALAXIES_NUMBER: number = 1;
export const GALAXIES_LAYOUT_NUMBER: number = 1;

// SOLAR_SYSTEMS CONFIG
export const SOLAR_SYSTEMS_NUMBER: number = 1;
export const SOLAR_SYSTEMS_LAYOUT_NUMBER: number = 2;
export const PLANETS_IN_SOLAR_SYSTEM_NUMBER: number = 4;
export const PLANETS_RATE_IN_SOLAR_SYSTEM_NUMBER: number = 4;
export const LAYOUTS_IN_SOLAR_SYSTEM: number = 7;
// PLANET CONFIG
